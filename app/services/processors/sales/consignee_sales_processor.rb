...
module Processors
  module Sales
    ...
    class ConsigneeSalesProcessor < BaseSalesProcessor
      attr_accessor :not_found_customers, :not_found_customers, :not_found_sales
      ...
      def import_row(row)
        ...
        sale = nil
        ProfitCenterVariant.where(sap_id: pcv.sap_id).each do |pcv|
          sale = find_sale(customer, sales_time, pcv, currency, row[:revenues].to_f)
